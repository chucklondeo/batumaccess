...
function batum_core_polylang_post_types($post_types) {
    foreach (['batum_product', 'batum_solution', 'batum_project', 'batum_download', 'batum_faq'] as $type) {
...
function batum_core_polylang_taxonomies($taxonomies) {
    foreach (['product_category', 'product_series', 'applications', 'download_category', 'industries', 'content_cluster', 'faq_category'] as $tax) {
        $taxonomies[$tax] = $tax;
    }
    return $taxonomies;
}
