    ...
    <main className="min-h-screen px-5 py-12 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <p className="mt-5 max-w-3xl text-sm leading-7 text-steel">
          Static CMS for the current Hostinger deployment. This site has no server or database, so nothing typed here goes live by itself: draft changes in this browser, then <strong className="text-white">Export JSON</strong> and hand it to whoever commits site updates.
        </p>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-steel">
          Document uploads work the same way: the file picker below only previews a document in this browser tab — it is not saved anywhere. To publish a real datasheet, add the file to <code className="rounded bg-white/10 px-1.5 py-0.5 text-white">public/docs/</code> in the project, set the product&apos;s <code className="rounded bg-white/10 px-1.5 py-0.5 text-white">document</code> path in <code className="rounded bg-white/10 px-1.5 py-0.5 text-white">data/site.ts</code> to match, and commit both.
        </p>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-steel">
          The search content seed list below is expanded automatically once a day by a GitHub Action (<code className="rounded bg-white/10 px-1.5 py-0.5 text-white">.github/workflows/seo-daily.yml</code>), which opens a pull request for review rather than publishing directly.
        </p>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-steel">
          Pages: {pageSlugs.join(", ")}. Adding or removing a page is a code change, not something this panel can do live — see the &quot;Adding or removing a page&quot; section in the project README.
        </p>
        ...
        <section className="mt-10 grid gap-5">
          {items.map((item) => (
            <article key={item.id} className="glass-card grid gap-4 rounded-3xl p-5 lg:grid-cols-[1fr_1fr_1fr_auto]">
              ...
              <label className="grid gap-2 text-sm font-semibold text-white">
                Product document (preview only — see note above)
